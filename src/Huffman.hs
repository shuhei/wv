...
data Tree a
  ...
  | Branch (Tree a) (Tree a) Integer Integer
  deriving (Show, Eq, Ord)
...
index :: Tree a -> Integer
index (Branch _ _ i _) = i

-- | Merge two trees with an index.
merge :: Integer -> Tree a -> Tree a -> Tree a
...
encode :: (Eq a, Hashable a) => Tree a -> [Code] -> [Integer] -> HashMap a Encoded -> HashMap a Encoded
